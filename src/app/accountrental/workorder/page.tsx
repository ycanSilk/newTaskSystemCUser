...
const WorkOrderListPage = () => {
  ...
  const handleViewDetail = (ticket_id: number, ticket_no: string) => {
    router.push(`/accountrental/workorder/deatil/${ticket_id}?ticket_no=${ticket_no}&ticket_id=${ticket_id}`);
  };
    ...
    if (workOrders.length === 0) {
      return (
        <div className="bg-white p-8 text-center">
          ...
          <Button 
            type="primary" 
            onClick={() => router.push('/accountrental/workorder/create')} 
            size="small" 
    ...
    return workOrders.map((workOrder) => (
      ...
      >
        ...
        <div className="flex justify-between items-start mb-3">
          <div>
            <p className="text-sm font-semibold">工单ID: {workOrder.ticket_id}</p>
            <p className="text-xs mb-1">工单编号: {workOrder.ticket_no}</p>
            <p className="text-xs">订单ID: {workOrder.order_id}</p>
          </div>
          <Tag color={statusColorMap[workOrder.status]} className="text-xs">
        ...
        <div className="mb-3 space-y-1">
          <p className="text-sm font-medium">工单标题：{workOrder.title}</p>
          <p className="text-xs line-clamp-2">工单描述：</p>
          <p className='text-xs line-clamp-2 bg-blue-50 border border-blue-200 p-2 rounded-md'>{workOrder.description}</p>
        </div>

        {/* 订单信息 */}
        <div className="mb-3 grid grid-cols-2 gap-2 text-xs">
          <div className="bg-gray-50 p-2 rounded-md">
            <p className="font-medium">订单金额</p>
            <p className="text-red-500">¥{workOrder.order_amount}</p>
          </div>
          <div className="bg-gray-50 p-2 rounded-md">
            <p className="font-medium">订单天数</p>
            <p>{workOrder.order_days} 天</p>
          </div>
          <div className="bg-gray-50 p-2 rounded-md">
            <p className="font-medium">订单状态</p>
            <p>{workOrder.order_status_text}</p>
          </div>
          <div className="bg-gray-50 p-2 rounded-md">
            <p className="font-medium">我的角色</p>
            <p>{workOrder.my_role === 'buyer' ? '买家' : '卖家'}</p>
          </div>
        </div>

        {/* 参与人信息 */}
        <div className="mb-3 text-xs">
          <p className="font-medium mb-1">参与人</p>
          <div className="flex gap-2">
            <Tag color="blue">买家: {workOrder.buyer_username}</Tag>
            <Tag color="green">卖家: {workOrder.seller_username}</Tag>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2">
          <Button
            ...
            className="text-blue-600 text-xs  border border-blue-600 rounded-sm px-3 py-1"
          >
            发送消息
          </Button>
          {workOrder.status === 0 && (
            <Button
              ...
              className="text-xs bg-green-600 border-green-600 rounded-sm px-3 py-1"
            >
              关闭工单
            </Button>
          )}
