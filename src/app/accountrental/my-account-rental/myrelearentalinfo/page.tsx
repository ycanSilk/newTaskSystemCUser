...
import {RentalOffer,  statusTextMap, statusToTabKeyMap, GetOffersRentalMarketListResponse, RentalOfferStatus} from '@/app/accountrental/types/rental/myrelearentalinfoTypes';
...
const RentalOfferPage = () => {
  ...
  const tabItems: TabsProps['items'] = [
    ...
    { key: 'ACTIVE', label: '已上架', children: null },
  ...
  const toggleOfferStatus = async (offerId: number, status: number) => {
    ...
    try {
      const response = await fetch('/api/rental/rentOut/toggleOffersRentalInfoStatus', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
