...
const publicPaths = ['/commenter/auth/login', '/commenter/auth/register', ];
...
const validateToken = async (token: string, origin: string): Promise<boolean> => {
  try {
    ...
    console.log('请求中间件状态码',response.status)
    console.log('请求校验tokenAPI的结果',response.json())
  } catch (error) {
    // 如果发生错误，说明Token无效
    return false;
  }
...
export async function middleware(request: NextRequest) {
  ...
  if (pathParts.length > 0 && isEncryptedRoute(pathParts[0])) {
    try {
      ...
      // 构建完整的新路径
      // 例如，解密后是'/commenter'，剩余路径是'dashboard'，则新路径是'/commenter/hall'
  ...
  console.log('从浏览器获取的Commenter_token:', token);
  // 如果没有Token，说明用户未登录，重定向到登录页
  if (!token) {
    // 确定重定向的登录页面
    // 无论什么路径，未登录都跳转到commenter登录页
  ...
  if (!isValidToken) {
    // 确定重定向的登录页面
    // 无论什么路径，Token无效都跳转到commenter登录页
    const loginPath = '/commenter/auth/login';
